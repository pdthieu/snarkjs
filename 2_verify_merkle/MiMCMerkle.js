...
module.exports = {
  buildMiMCMerkle: async function() {
    const mimcjs = await buildMimc7();
    const babyJub = await buildBabyjub();
    const F = babyJub.F;
    return new MiMCMerkle(mimcjs, F);
  }
}

class MiMCMerkle {
  constructor(mimcjs, F) {
    this.mimcjs = mimcjs;
    this.F = F;
  }
  ...
  rootFromLeafAndPath(leaf, idx, merkle_path) {
    if (merkle_path.length > 0) {
      ...
      root[0] = this.mimcjs.multiHash([left, right], 2);
      ...
      for (var i = 1; i < depth; i++) {
        ...
        root[i] = this.mimcjs.multiHash([left, right], 2);
      }
      return root[depth - 1];
    } else {
      return leaf;
  ...
  pairwiseHash(array) {
    if (array.length % 2 == 0) {
      ...
      for (var i = 0; i < array.length; i = i + 2) {
        arrayHash.push(this.mimcjs.multiHash([array[i], array[i + 1]], 2));
      }
      return arrayHash;
    } else {
